...
import type { HotspotGridResult } from '../../src/services/canvas/HotspotGrid';
...
function makeHotspotGrid(
  cellOverrides: Array<Partial<HotspotGridResult['cells'][number]>> = [],
): HotspotGridResult {
  const cells: HotspotGridResult['cells'] = [];
  ...
  return {
    gridSize: 8,
    cells,
    logicalBounds: { x: 0, y: 0, width: 4096, height: 4096 },
  };
}
...
describe('CanvasVisionService', () => {
  ...
  describe('transcribe', () => {
    ...
    it('includes hotspot grid stroke hints in system prompt', async () => {
      ...
      const grid = makeHotspotGrid([
        { row: 2, col: 3, strokeIds: ['s1', 's2'] },
        { row: 5, col: 1, strokeIds: ['s3'] },
      ]);
      ...
      expect(systemText).toContain('(2,3): strokes [s1, s2]');
      expect(systemText).toContain('(5,1): strokes [s3]');
    });
  ...
  describe('buildSystemPrompt', () => {
    ...
    it('includes only non-empty cells', () => {
      const grid = makeHotspotGrid([
        { row: 0, col: 0, strokeIds: ['a'] },
      ]);
      const prompt = service.buildSystemPrompt(grid);
      expect(prompt).toContain('(0,0): strokes [a]');
