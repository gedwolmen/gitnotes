...
| [Todo Pull — Parse Resilience](./todo-parse-resilience.md) | `pullTodosFromRepo` now detects non-JSON content before parsing, logs parse failures at error level with the file path, and reports a `Skipped N todo file(s)` summary instead of silently dropping todos (#1008) |
