...
| [Foreground Pull Health — Settings Indicator](./foreground-sync-health.md) | `getForegroundSyncHealth()` + `useForegroundSyncHealth()` + a "Last sync failed · Nm ago" row in Settings → Sync engine, so a stalled/failed foreground pull is no longer invisible (#1007) |
