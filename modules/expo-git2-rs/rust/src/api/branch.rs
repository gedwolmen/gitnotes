...
pub fn list_branches(repo_path: &str) -> Result<Vec<BranchEntry>, GitError> {
    ...
    for branch_result in
        repo.branches(Some(BranchType::Local)).map_err(|e| map_git_error(e))?
    {
        let (branch, _) = branch_result.map_err(|e| map_git_error(e))?;
        let name = branch
            .name()
            .map_err(|e| map_git_error(e))?
            .unwrap_or("")
...
pub fn create_branch(
    ...
    let commit_oid_val = repo.revparse_single(commit_oid)
        .map_err(|e| map_git_error(e))?
        .id();
    let commit = repo.find_commit(commit_oid_val)
        .map_err(|e| map_git_error(e))?;
...
pub fn checkout_branch(repo_path: &str, branch_name: &str) -> Result<(), GitError> {
    ...
    let commit = branch.get().peel_to_commit().map_err(|e| map_git_error(e))?;
...
pub fn delete_branch(repo_path: &str, branch_name: &str) -> Result<(), GitError> {
    ...
    branch.delete().map_err(|e| map_git_error(e))
}
