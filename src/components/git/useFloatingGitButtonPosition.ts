...
export interface FloatingGitButtonPositionState {
...
export function useFloatingGitButtonPosition(): FloatingGitButtonPositionState {
  ...
  const initialPosition = resolveFloatingGitButtonPlacement(
  ...
  const applyPosition = useCallback((position: FloatingButtonPosition) => {
    ...
    publishButtonRect('stage', { x: position.x, y: position.y, size: GIT_BUTTON_SIZE });
  ...
  const savePosition = useCallback((position: FloatingButtonPosition) => {
    AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(position)).catch((error: unknown) => {
      console.warn('Failed to save git button position:', error);
    });
  }, []);
    ...
    AsyncStorage.getItem(STORAGE_KEY).then((storedPosition) => {
      ...
      if (
        ...
        try {
          ...
          console.warn('Failed to restore git button position:', error);
        }
      }
