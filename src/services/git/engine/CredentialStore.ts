...
async function doRefresh(
  ...
  return refreshOAuthToken(refreshToken, 'github', null);
}
...
function toInternalCredential(stored: StoredCredential): { kind: string; username?: string; privateKey?: string; publicKey?: string | null; passphrase?: string | null; token?: string } {
  ...
  if (c.kind === 'github_classic_pat' || c.kind === 'github_fine_grained_pat' || c.kind === 'pat') {
    return {
      kind: 'token',
...
async function handleOAuthCredential(
  ...
  if (existing) {
    ...
    return oauthResultToDoneClaim(result, repoId, credential);
  }
...
function oauthResultToDoneClaim(
  ...
  storage.set(credKey(repoId), JSON.stringify(updatedCredential)).catch(() => undefined);
  if (result.result.expiresAt) {
    storage.set(expiryKey(repoId), result.result.expiresAt).catch(() => undefined);
  }
